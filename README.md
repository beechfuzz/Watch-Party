...
`sync_soft_drift`/`sync_hard_drift` must carry an explicit unit here (e.g. `"300ms"`) — unlike every other duration setting in this file, a bare number is rejected rather than silently treated as seconds, since that would turn a sub-second drift threshold into a multi-minute one. If `config.jsonc` doesn't exist, the server starts in a **setup-required mode** and serves a setup wizard at that same address: visit it in a browser, fill in the form (pre-filled with the defaults above), and submit — it validates every field with the same rules the loader itself uses, atomically writes `config.jsonc`, and generates a `TOKEN_ENCRYPTION_KEY` for you to copy and set yourself (shown once; never written to `config.jsonc` or anywhere else on disk). `GET /healthz` returns `200` throughout; every other route serves the wizard's own `503` until a config file exists. Once the file is written, the server switches to normal mode automatically if `TOKEN_ENCRYPTION_KEY`/`TOKEN_ENCRYPTION_KEY_FILE` is already set in your environment; otherwise it waits for you to set one of those two variables and restart it. See `ARCHITECTURE.md` §16 for the full design.
