...
- **Media authorization is re-validated per participant, on join — not inherited from the host.** Being the host, or another member already having access, never implicitly grants anyone else access to an item they couldn't already see in Emby themselves.
...
go test -race ./...
node --test internal/webassets/web/static/js/sync.test.mjs    # frontend sync-math tests, mirrors internal/syncalg
